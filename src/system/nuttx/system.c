...
//
// NuttX system layer for zenoh-pico.
// Uses POSIX pthreads, BSD sockets (via rpmsgdrv virtual eth), and arc4random.
...
unsigned long zp_clock_elapsed_us_since(z_clock_t *instant, z_clock_t *epoch) {
    long elapsed = (1000000L * (instant->tv_sec - epoch->tv_sec) + (instant->tv_nsec - epoch->tv_nsec) / 1000L);
    return elapsed > 0 ? (unsigned long)elapsed : 0;
}

unsigned long zp_clock_elapsed_ms_since(z_clock_t *instant, z_clock_t *epoch) {
    long elapsed = (1000L * (instant->tv_sec - epoch->tv_sec) + (instant->tv_nsec - epoch->tv_nsec) / 1000000L);
    return elapsed > 0 ? (unsigned long)elapsed : 0;
}

unsigned long zp_clock_elapsed_s_since(z_clock_t *instant, z_clock_t *epoch) {
    long elapsed = (long)(instant->tv_sec - epoch->tv_sec);
    return elapsed > 0 ? (unsigned long)elapsed : 0;
}
...
/*------------------ Time (CLOCK_REALTIME) ------------------*/
// z_time_t = struct timespec; use CLOCK_REALTIME for wall-clock functions.
...
unsigned long z_time_elapsed_us(z_time_t *time) {
    z_time_t now;
    clock_gettime(CLOCK_REALTIME, &now);
    long elapsed = (1000000L * (now.tv_sec - time->tv_sec) + (now.tv_nsec - time->tv_nsec) / 1000L);
    return elapsed > 0 ? (unsigned long)elapsed : 0;
}
...
unsigned long z_time_elapsed_ms(z_time_t *time) {
    z_time_t now;
    clock_gettime(CLOCK_REALTIME, &now);
    long elapsed = (1000L * (now.tv_sec - time->tv_sec) + (now.tv_nsec - time->tv_nsec) / 1000000L);
    return elapsed > 0 ? (unsigned long)elapsed : 0;
}
...
unsigned long z_time_elapsed_s(z_time_t *time) {
    z_time_t now;
    clock_gettime(CLOCK_REALTIME, &now);
    long elapsed = (long)(now.tv_sec - time->tv_sec);
    return elapsed > 0 ? (unsigned long)elapsed : 0;
}
